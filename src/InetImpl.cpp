...
namespace inet {
   ...
   string InetException::what(void) const noexcept{
      return errorMessage;
   }

   InetClosedByHostException::InetClosedByHostException(const string&  errString)
      : errorMessage{errString}
   { }
   ...
   ssize_t Inet::readBuffer(size_t len, Handler* hdlr, void** buff) anyexcept{
      ...
      if(readLen == 0)                  throw InetClosedByHostException("Connection was closed by the server.");
